...
class ControllEmployee extends EmployeeAbs {
    ...
    async SetEmployee(req, res) {
        try {
            const { name, profession, img, services, subServices, workStart, workEnd, mainBreakStart, mainBreakEnd, defaultBreakBetweenServices } = req.body;
            ...
            const newEmployee = await Employee.create({
                ...
                defaultBreakBetweenServices: defaultBreakBetweenServices ?? salon.defaultBreakBetweenServices
            });
            res.status(201).json({
                success: true,
    ...
    async UpdateEmployee(req, res) {
        try {
            ...
            if (workStart)
                updateData.workStart = workStart;
            if (workEnd)
                updateData.workEnd = workEnd;
            if (mainBreakStart !== undefined)
                updateData.mainBreakStart = mainBreakStart;
            if (mainBreakEnd !== undefined)
                updateData.mainBreakEnd = mainBreakEnd;
            if (defaultBreakBetweenServices !== undefined)
                updateData.defaultBreakBetweenServices = defaultBreakBetweenServices;
            // 3️⃣ Update employee
            const updatedEmployee = await Employee.findByIdAndUpdate(employeeId, updateData, { new: true } // return the updated document
