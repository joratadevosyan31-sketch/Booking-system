...
class BookingController extends BookingAbs {
    ...
    async setBooking(req, res) {
        try {
            const { service, subServices, employee, 
            // customer,
            date, startTime } = req.body;
            ...
            const servicesDuration = subServiceDocs.reduce((sum, s) => sum + s.duration, 0);
            const breakBetween = employeeData.defaultBreakBetweenServices || 5;
            const totalBreakTime = breakBetween * (subServiceDocs.length - 1);
            const endTime = calculateEndTime(startTime, servicesDuration + totalBreakTime);
    ...
    async updateBooking(req, res) {
        try {
            const { bookingId, service, subServices, employee, customer, date, startTime } = req.body;
            ...
            if (subServices && Array.isArray(subServices)) {
                ...
                const employeeData = await Employee.findById(employee || updateData.employee);
                const breakBetween = employeeData?.defaultBreakBetweenServices || 5;
                const servicesDuration = subServiceDocs.reduce((sum, s) => sum + s.duration, 0);
                const totalBreakTime = breakBetween * (subServiceDocs.length - 1);
                updateData.endTime = calculateEndTime(startTime || updateData.startTime, servicesDuration + totalBreakTime);
            }
